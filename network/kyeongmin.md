...
1. 데이터 암호화
  - SSL/TLS 프로토콜을 사용하여 웹 트래픽을 암호화함으로써 클라이언트와 서버 간 데이터를 보호
2. 데이터 무결성
  - 전송되는 데이터가 변경되지 않도록 보호
  - 암호화된 데이터는 해시 함수를 통해 무결성을 확인하고 데이터 전송 중에 수정되지 않았는지 확인
3. 인증
  - 서버의 신원을 확인하여 중간자 공격 등을 방지

## TLS/SSL 핸드셰이크 과정은 어떻게 이루어지나요?
### TLS/SSL 핸드셰이크
- TLS 암호화를 사용하는 통신 세션을 실행하는 프로세스
- 사용자가 HTTPS를 통해 웹 사이트를 탐색하고 브라우저가 처음 웹 사이트의 원본 서버 쿼리를 시작할 때 발생
### 동작 과정
1. 클라이언트 헬로 메시지: 클라이언트가 클라이언트의 정보(TLS 버전, 암호화 알고리즘 등)를 담아 서버로 ‘헬로’ 메시지를 전송
2. 서버 헬로 메시지: 클라이언트 헬로 메시지에 대한 응답으로 서버에 대한 정보(사용할 TLS 버전, 암호화 알고리즘 등)를 포함하여 메시지 전송
3. 서버 인증: 서버의 인증서를 클라이언트에게 전송, 해당 인증서에는 서버의 공개키와 서버 신원을 증면하는 인증기관(CA)의 서명 포함
4. (optional) 서버 키 교환: 선택한 암호화 방식에 따라 추가적인 키 교환 정보 필요
5. 서버 완료: 서버는 클라이언트가 추가적인 메시지를 보낼 수 있다고 알려줌
6. 클라이언트 키 교환: 클라이언트는 서버의 공개키를 사용하여 세션키를 암호화하고 서버에 전송, 세션키는 데이터 암호화에 사용
7. 완료 메시지: 클라이언트는 암호화된 연결을 사용한다고 서버에 알림, 이후의 메시지가 암호화되어 전송
8. 클라이언트 완료: 클라이언트는 핸드셰이크 과정이 완료되었다는 메시지 전송
9. 서버 완료: 서버도 암호화된 연결을 사용한다 알리고 핸드셰이크가 완료됐다는 메시지 전송

## MITM(중간자 공격)이란 무엇이며, 이를 방어하는 방법은 무엇인가요?
### MITM(중간자 공격)
- 공격자가 사용자의 인터넷 서버와 해당 인터넷 트래픽의 목적지 사이에 끼어들어 데이터 전송을 가로채는 공격
- 와이파이 핫스팟 등을 이용하여 데이터 탈취 가능
- 유형
  - IP 스푸핑: 네트워크 취약점을 이용하여 다른 사람의 IP를 탈취하는 공격 방식
  - 이메일 하이재킹: 이메일에 포함된 중요 데이터를 탈취하는 공격 방식
  - MITB 공격: 트로이 목마(트래픽을 리다이렉션하여 사용자에게 가짜 웹사이트를 표시하고 사용자가 가짜 웹사이트에 정보를 입력하도록 유도)로 해킹하는 공격방식
- 예방 방법
  - HTTPS 사용
  - 디지털 인증서 사용
  - VPN 사용
