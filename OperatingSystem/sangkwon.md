...
- I/O 바운드(I/O-bound) 작업: I/O 바운드 작업은 네트워크 요청, 파일 입출력, 데이터베이스 조회처럼 I/O 작업에 많은 시간을 소비하는 경우입니다. 이러한 작업은 대기 시간이 길기 때문에 CPU가 실제로 작업을 수행하지 않고 대기 상태에 놓이는 경우가 많습니다. 그렇기에 스레드 수 = CPU 코어 수 * (1 + I/O 대기시간 / CPU사용시간)
