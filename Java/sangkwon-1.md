...
- 애플리케이션 타입: 스프링 애플리케이션이 시작될 때 빈이 생성되고, 애플리케이션이 종료될 때 소멸됩니다.
...
- POJO란 Plain Old Java Object로써 순수 자바 객체를 의미합니다. POJO는 특정 프레임워크나 라이브러러리에 종속되지 않으며 상속이나 인터페이스를 구현하지 않는 구조입니다. 즉 클래스, 필드, 메서드로 이루어진 간단한 구조입니다.
- 스프링 프레임워크에서의 POJO는 일반적으로 도메인 객체입니다.

## 도메인 엔티티 객체에서 해당 객체가 JPA 엔티티로 되어 있을 때 해당 객체는 POJO일까요?
- JPA 엔티티는 특정 인터페이스를 구현하거나 특정 클래스를 상속 받을 필요가 없으며 기본적인 클래스 구조 형태를 가지기에 POJO에 가깝습니다. 하지만 JPA 라이브러리에 종속적인 부분이 있기에 완전한 POJO는 아닙니다.

## Spring Web MVC에서 요청 마다 Thread가 생성되어 Controller를 통해 요청을 수행할텐데, 어떻게 1개의 Controller만 생성될 수 있을까요?
1. 먼저 컨트롤러는 싱글톤 형태로 스프링 컨테이너에서 관리됩니다.
2. 톰캣 환경에서는 쓰레드 풀이 존재하며 요청 시 해당 쓰레드 풀에서 관리되는 각 쓰레드가 할당됩니다.
3. 이때 각 쓰레드는 이미 생성된 동일한 Controller 인스턴스를 사용하며 사용이 끝나도 여전히  Controller 인스턴스가 존재합니다.

## Spring WEB MVC의 근간에는 Java Servlet 이 있는데요. Spring 은 Servlet을 어떻게 구성해서 이를 구현했을까요?
- DispatcherServlet을 기반으로 MVC 패턴을 구현했습니다. 모든 HTTP 요청을 DispatcherServlet이 제일 먼저 받으며 그 후 해당 요청을 처리할 컨트롤러를 찾습니다. 먼저 HandlerMapping을 이용하여 요청 경로를 가지고 있는 컨트롤러 클래스를 찾습니다. 그후 HandlerAdapter를 이용하여 요청 경로의 Http Method를 가지는 (기존 컨트롤러 클래스) 메서드를 찾습니다. 그리하여 해당 비즈니스 로직을 처리하며 결과 값이 view가 있는 경우 view resolver를 이용하여 처리합니다.

## Filter는 Servlet의 스펙이고, Interceptor는 Spring MVC의 스펙입니다. Spring Application에서 Filter와 Interceptor를 통해 예외를 처리할 경우 어떻게 해야 할까요?
- 필터의 경우 try - catch 문으로 예외 처리를 진행하고 인터셉터는 @ControllerAdvice를 이용합니다.

## @ControllerAdvice 이름을 보면은 컨트롤러 레이어와 관련된 예외 처리를 진행하는것으로 보이는데 인터셉터의 경우도 @ResetControllerAdvice, @ControllerAdvice를 이용하나요?
- 두 어노테이션은 Spring Application 전범위에서 발생하는 예외들에 대해서 처리하는 것이기 때문에 사용해도 상관 없다고 생각합니다.

## Spring Application을 구동할 때 메서드를 실행시키는 방법에 대해 설명해주세요.
- CommandLineRunner, ApplicationRunner, InitializingBean 인터페이스를 구현한 방법이 있으며 @PostConsturctor, @EventListener 처럼 어노테이션 기반의 방식이 있습니다.

## 의존성과 설정값을 생성자 인자로 주입해야 하는 이유에 대해 설명해주세요.
- 의존성 주입이 강제화되고 불변성이 보장되기 때문입니다. 생성자가 호출되는 시점에 의존하는 객체를 주입기에 필요로 하는 의존 객체들을 주입할 수 있습니다. 또한 생성자가 호출되는 시점에 설정 값들을 주입 받았기 때문에 이후 값이 변경됨에도 기존의 값들을 유지할 수 있어서 생성자 주입을 권고합니다.

## JPA 영속성 컨텍스트의 이점(5가지)을 설명해주세요.
- 1차 캐시: 데이터베이스에서 조회된 엔티티는 먼저 1차 캐시에 저장됩니다. 그 후 동일한 트랜잭션에서 엔티티를 한번 혹은 여러번 조회할 때 데이터베이스가 아닌 1차 캐시에서 조회하여 DB 부하를 줄입니다.
- 변경 감지: 커밋 되는 시점에 엔티티가 초기 상태와 달라졌다면 JPA에서 수정 부분만을 반영합니다. 이에 따라 개발자가 수동으로 엔티티의 변경사항을 저장하는 코드를 제거할 수 있습니다.
- 쓰기 지연: JPA에서 쓰기 작업을 진행할 때 동일 트랜잭션에서 여러 시점에 쓰기를 하는 것이 아닌 한번에 처리하는 것을 말합니다. 이에 따라 효율적으로 DB Connection을 진행할 수 있습니다.
- 엔티티 생명주기 관리: 엔티티의 비영속, 영속, 준영속, 삭제와 같은 엔티티의 상태에 대해서 관리하는 것을 말합니다. 이에 따라 개발자가 따로 엔티티의 생명주기나 상태를 관리할 필요가 없어집니다.
- 지연 로딩: 영속성 컨텍스트는 필요한 시점에만 연관된 엔티티를 로딩하기에 불필요한 DB 부하를 방지합니다.

## JPA Propagation 전파단계를 설명해주세요.
- 먼저 트랜잭션 전파란 해당 트랜잭션이 실행될 때 다른 트랜잭션을 어떻게 처리할지에 대한 것을 말합니다.
- REQUIRED(기본 값): 이미 진행 중인 트랜잭션이 있으면 해당 트랜잭션을 그대로 사용합니다. 진행 중인 트랜잭션이 없으면 새로운 트랜잭션을 시작합니다.
- REQUIRES_NEW: 항상 새로운 트랜잭션을 시작합니다. 이미 진행 중인 트랜잭션이 있으면 해당 트랜잭션을 잠시 보류하고 새로운 트랜잭션을 시작한 후, 새 트랜잭션이 끝나면 기존 트랜잭션을 재개합니다.
- SUPPORTS: 트랜잭션이 있으면 해당 트랜잭션을 사용하고, 없으면 트랜잭션 없이 실행합니다.
- NOT_SUPPORTED: 트랜잭션이 있으면 잠시 중단하고, 트랜잭션 없이 작업을 수행합니다.
- MANDATORY: 반드시 기존의 트랜잭션이 존재해야 합니다. 현재 트랜잭션이 없으면 예외를 발생시킵니다.
- NEVER: 트랜잭션이 있으면 예외를 발생시킵니다. 트랜잭션이 없는 경우에만 실행됩니다.
- NESTED: 현재 진행 중인 트랜잭션이 있다면 그 안에서 중첩된 트랜잭션을 시작합니다. 만약 진행 중인 트랜잭션이 없다면 새로운 트랜잭션을 시작합니다.

## Propagation 단계별 활용 예시를 알려주세요
- **REQUIRED**: 대부분의 서비스 계층에서 기본으로 사용합니다. 부모 트랜잭션이 있으면 재사용하고, 없으면 새로 시작하기 때문에 가장 일반적입니다.
- **REQUIRES_NEW**: 이메일 발송과 같이 메인 트랜잭션이 롤백되더라도 별도의 트랜잭션에서 반드시 실행되어야 하는 작업에 사용됩니다.
- **SUPPORTS**: 트랜잭션이 꼭 필요하지 않은 작업에 사용됩니다.
- **NOT_SUPPORTED**: 특정 작업에서 트랜잭션을 아예 피하고자 할 때 사용합니다.
- **MANDATORY**: 반드시 트랜잭션이 있는 상황에서만 실행해야 하는 경우에 사용합니다.
- **NEVER**: 트랜잭션 환경에서 실행하면 안 되는 작업이 있을 때 사용합니다.
- **NESTED**: 부모 트랜잭션의 일부로 처리되지만, 특정 부분만 독립적으로 롤백하거나 커밋할 필요가 있는 경우 사용됩니다.

## JPA를 쓴다면 그 이유에 대해서 설명해주세요.
- 자바 코드를 이용해서 DB 관련 작업을 보다 객체지향적으로 진행하고 싶을 때 사용합니다. 또한 JPA에서 이미 CRUD 관련된 기능들에 대해서 추상화가 되어 있으며 특정 DB에 종속적이지 않게 사용할 수 있습니다. 마지막으로는 캐싱이나 지연로딩과 같은 성능 향상을 이유로 사용할 수 있습니다.

## N + 1 문제는 무엇이고 이것이 발생하는 이유와 이를 해결하는 방법을 설명해주세요.
- 먼저 N+1 문제는 엔티티를 조회하기 위해서 1개의 쿼리를 실행했을 때 추가적으로 N개의 쿼리가 실행되는 것을 말합니다. 부모 엔티티와 자식 엔티티가 있을 때 부모 엔티티 10개를 조회하기 위한 쿼리 1개를 실행했을 때 연관관계를 가진 자식들을 조회하는 쿼리 10개가 실행되어 1 + 10의 쿼리가 실행되는 것이 N+1 문제입니다.
- 해결 방법은 Fetch Join을 이용하는 것입니다. Join 문을 이용하여 연관관계를 가진 객체를 조회한 후 Fetch 키워드를 이용해서 즉시 로딩을 실행해 1개의 쿼리만 실행하도록 합니다. 두번째 방법은 Entity Graph를 이용해 연관된 엔티티를 로딩하는 방법입니다.
